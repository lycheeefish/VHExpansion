...
import torch
import os
from utils import DissimilarityLoss, encode_image, preprocess_image_llava_next, process_images_tensor
    ...
    if use_categories:
        ...
        for category in categories:
            ...
            for img_name in tqdm(os.listdir(category_path)):
                ...
                ## ori
                image = preprocess_image_llava_next(image).squeeze(0)
...
def apply_adv_perturbations(ori_img, img_name, output_category_path, img_rgb, epsilon, alpha, num_steps, method='i-fgsm', hallucination_type='non_h'):
    ...
    for step in range(1, num_steps + 1):
        ...
        ## ori
        adv_inputs = process_images_tensor([adv_img], image_processor, model.config, img_rgb, 'llava_next')
