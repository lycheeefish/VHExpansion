import torch
import os
from utils import DissimilarityLoss, preprocess_image_blip, encode_image
    ...
    if use_categories:
        ...
        for category in categories:
            ...
            for img_name in tqdm(os.listdir(category_path)):
                ...
                ## resize
                image = preprocess_image_blip(image)
...
def apply_adv_perturbations(ori_img, img_name, output_category_path, img_rgb, epsilon, alpha, num_steps, method='i-fgsm', hallucination_type='non_h'):
    ...
    adv_img = ori_img.clone().detach().to(device).requires_grad_(True)
    ...
    elif hallucination_type == 'non_h':
        random_perturbation = float(1/255)
        adv_img = torch.clamp(adv_img + random_perturbation, min=0, max=1).detach_()
    ...
    for step in range(1, num_steps + 1):
        ...
        ori_inputs = vis_processors["eval"](img_rgb).unsqueeze(0).to(device)
        with maybe_autocast():
            ori_embeddings = ln_vision(visual_encoder(ori_inputs))

        ## resize
        adv_inputs = encode_image(adv_img).to(device)
        with maybe_autocast():
            adv_embeddings = ln_vision(visual_encoder(adv_inputs))
